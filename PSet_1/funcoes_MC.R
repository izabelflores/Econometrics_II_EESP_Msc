...
results <- foreach(
  ...
  for (m in 1:M) {
    ...
    resultsT$bias[m] <- ma1$coef[a] - parametro
    
    # Store the normalized coefficient
    resultsT$normalized_coef[m] <- (ma1$coef[a] - parametro) / sqrt(ma1$var.coef[a,a])
    
    # Store the test decision
    resultsT$reject[m] <- as.numeric(
      abs((ma1$coef[a] - parametro) / sqrt(ma1$var.coef[a,a])) >= qnorm(0.975)
    )
...
gg1 <- ggplot(data = probs, aes(x = capT)) +
...
i_vec <- which(capT_vec %in% c(30, 45, 100, 200, 300, 500))
...
# Create a plot with the results
gg3 <- ggplot(data = rej_rate, aes(x = capT)) +
  theme_bw(base_size = 25) +
  theme(plot.margin = unit(c(5, 7, 2, 2), "mm")) +
